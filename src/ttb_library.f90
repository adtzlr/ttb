...
integer,private,parameter :: wp=kind(0.0d0)
    ...
    type Tensor1
        ! tensor of rank 1
        real(kind=wp), dimension(3) :: a = 0.d0
    end type Tensor1
     
    type Tensor2
        ! tensor of rank 2
        real(kind=wp), dimension(3,3) :: ab = 0.d0
    end type Tensor2
     
    type Tensor2S
        ! symmetric tensor of rank 2 stored as vector
        real(kind=wp), dimension(6) :: a6 = 0.d0
    end type Tensor2S
  
    type Tensor4
        ! tensor of rank 4
        real(kind=wp), dimension(3,3,3,3) :: abcd = 0.d0
    end type Tensor4
     
    type Tensor4S
        ! symmetric tensor of rank 4 stored as 6x6 matrix
        real(kind=wp), dimension(6, 6) :: a6b6 = 0.d0
    end type Tensor4S
...
!------DOT SECTION-----------------------------------------
       include 'libdot.finc'
!------DOUBLE DOT SECTION----------------------------------
       include 'libddot.finc'
!------POWER SECTION---------------------------------------
       include 'libpower.finc'
!------DYADIC SECTION--------------------------------------
       include 'libdyadic.finc'
!------CROSS-DYADIC SECTION--------------------------------
       include 'libcrossdyadic.finc'
!------PERMUTE SECTION-------------------------------------
       include 'libpermute.finc'
!------TENSOR FUNCTION SECTION-----------------------------
       include 'libtransp.finc'
       include 'libtrace.finc'
       include 'libdet.finc'
       include 'libdev.finc'
       include 'libunimodular.finc'
       include 'libinv.finc'
       include 'libnorm.finc'
       include 'libsqrt.finc'
       include 'librotation.finc'
!------TENSOR INITIALIZATION SECTION-----------------------
       include 'libidentity.finc'
!------SYMSTORE SECTION------------------------------------
       include 'libsymstore.finc'
!------USYMSTORE SECTION-----------------------------------
       include 'libtenstore.finc'
!------STRAINSTORE SECTION---------------------------------
       include 'libstrainstore.finc'
!------REDUCE DIM. SECTION---------------------------------
       include 'libreducedim.finc'
!------AS ARRAY SECTION------------------------------------
       include 'libasarray.finc'
       include 'libasabqarray.finc'
!------PIOLA SECTION---------------------------------------
       include 'libpiola.finc'
!------END FUNCTIONS---------------------------------------
!------BEGIN SUBROUTINES-----------------------------------
       include 'libassignscalar.finc'
       include 'libassignarray.finc'
       include 'libassignten2sym.finc'
